...
// Image Object
function ObjImage(n,i,a,x,y,w,h,v,z,d,t,cl) {
  this.name = n
  this.altName = a
  this.x = x
  this.y = y
...
function ObjImageBuild() {
	...
	if(this.bIsWCAG && !this.hasOnUp){
		this.divInt += ' href="javascript:void(null)"';
	}
	else if(this.hasOnUp && (is.iOS || this.bIsWCAG) && !is.ie) this.divInt += ' href="javascript:' + this.name + '.up()"'
...
function ObjImageBuildSvg(){
	...
	{	
		...
		this.divReflect = addReflection(this.name, this.imgSrc, this.reflectedImageX, this.reflectedImageY, this.reflectedImageWidth, 
	...
	if(this.hasOuterShadow)
	{
		...
		if(xOffset <= 0 || yOffset <= 0)
		{	
			this.divInt += '<image name="'+this.name+'Img" id="'+this.name+'Img" xlink:href = "' + this.imgSrc + '" preserveAspectRatio="none"';
...
function ObjImageActivate() {
  ...
  if( this.capture & 4  && !is.iOS && !this.bIsWCAG) {
